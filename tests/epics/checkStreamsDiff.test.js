...
import { checkDiffStreamsMoreStreamsInPresentState, checkDiffStreamsAndPriorizeMainStreamerState, checkDiffStreamsMoreStreamsInPastState } from '../fixtures/checkStreamsDiff';
...
describe('checkFetchStreamsDiffEpic', () => {
  ...
  beforeAll(async () => {
    ...
    it('should add streams (priorizing mainStreamer) to queue and show first one when streams status was checked and it has difference', async () => {

      const action$ = ActionsObservable.of({
          type: CHECK_DIFF_STREAMS
      });
      
      const state$ = new StateObservable(new Subject(), checkDiffStreamsAndPriorizeMainStreamerState)
